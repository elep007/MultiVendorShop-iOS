...
import Toaster
class BuyerOneShopVC: UIViewController,UITableViewDelegate, UITableViewDataSource {
    var loginVC : LoginVC!
    ...
    @IBOutlet weak var shopMail: UILabel!
    @IBOutlet weak var shopAddress: UILabel!
    @IBOutlet weak var shopFavImg: UIImageView!
    ...
    func setready(){
        shopid = Defaults.getNameAndValue(Defaults.SHOPID_KEY)
        shopname = Defaults.getNameAndValue(Defaults.SHOPNAME_KEY)
        shopemail = Defaults.getNameAndValue(Defaults.SHOPEMAIL_KEY)
        shopmobile = Defaults.getNameAndValue(Defaults.SHOPMOBILE_KEY)
        shopaddress = Defaults.getNameAndValue(Defaults.SHOPADDRESS_KEY)
        shoplocation = Defaults.getNameAndValue(Defaults.SHOPLOCATION_KEY)
        shopimage = Defaults.getNameAndValue(Defaults.SHOPIMG_KEY)
        shopfav = Defaults.getNameAndValue(Defaults.SHOPFAV_KEY)
        shoptitle.text = shopname
        let ImgUrl = Global.imageUrl + shopimage
        shopImg.sd_setImage(with: URL(string: ImgUrl), completed: nil)
        shopName.text = shopname
        shopTelephone.text = shopmobile
        shopMail.text = shopemail
        shopAddress.text = shopaddress
        if(shopfav == "yes"){
            shopFavImg.image = UIImage.init(named: "fav-red")
        }
    }
    func getproduct(){
        ...
        allproduct = []
        let parameters: Parameters = ["userid":Defaults.getNameAndValue(Defaults.USERID_KEY), "shopid":Defaults.getNameAndValue(Defaults.SHOPID_KEY)]
        
        Alamofire.request(Global.baseUrl + "buyershopproduct.php", method: .post, parameters: parameters, encoding:JSONEncoding.default).responseJSON{ response in
            print(response)
    ...
    @IBAction func addFavBtn(_ sender: Any) {
        if(Defaults.getNameAndValue(Defaults.SHOPFAV_KEY) == "no"){
            let parameters: Parameters = ["userid":Defaults.getNameAndValue(Defaults.USERID_KEY), "shopid":Defaults.getNameAndValue(Defaults.SHOPID_KEY)]
            
            Alamofire.request(Global.baseUrl + "buyersetfavuser.php", method: .post, parameters: parameters, encoding:JSONEncoding.default).responseJSON{ response in
                print(response)
                if let value = response.value as? [String: AnyObject] {
                    print(value)
                    let status = value["status"] as! String
                    if status == "ok" {
                        ...
                        self.shopFavImg.image = UIImage(named: "fav-red")
                    }else{
                        self.spinnerView.endRefreshing()
                        Toast(text: "No product data").show()
                    }
    ...
    @IBAction func showAddress(_ sender: Any) {
        if let url = URL(string: "https://www.google.com/maps/@" + shoplocation + ",18z"), UIApplication.shared.canOpenURL(url) {
