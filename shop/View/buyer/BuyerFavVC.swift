...
import Toaster
import Alamofire
class BuyerFavVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    var loginVC : LoginVC!
    ...
    @IBOutlet weak var favuserTB: UITableView!
    @IBOutlet weak var favproductTB: UITableView!    
    @IBOutlet weak var sel_type: UISegmentedControl!
    @IBOutlet weak var productUV: UIView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        favuserTB.delegate = self
        favuserTB.dataSource = self
        favproductTB.delegate = self
        favproductTB.dataSource = self
        toastenvironment()
    }
    ...
    func setready(){
        ...
        Alamofire.request(Global.baseUrl + "buyerfavpage.php", method: .post, parameters: parameters, encoding:JSONEncoding.default).responseJSON{ response in
            print(response)
                ...
                if status == "ok" {
                    if status_shop == "ok"{
                        ...
                        for i in 0 ... (shops?.count)!-1 {
                            let fav_id = shops?[i]["favid"] as! String
                            let shop_id = shops?[i]["shopid"] as! String
                    ...
                    if status_product == "ok"{
                        ...
                        for i in 0 ... (products?.count)!-1 {
                            let fav_id = products?[i]["favid"] as! String
                            ...
                            self.favproductid.append(fav_id)
                        }
                    }else{
                        Toast(text: "No product data").show()
                    }
                    self.spinnerView.endRefreshing()
                    self.favuserTB.reloadData()
                    self.favproductTB.reloadData()
                }else{
                    self.spinnerView.endRefreshing()
                    Toast(text: "No product data").show()
                }
    ...
    @IBAction func sel_type(_ sender: Any) {
        switch sel_type.selectedSegmentIndex
        {
        case 0:
            productUV.isHidden = false
            userUV.isHidden = true
        case 1:
            productUV.isHidden = true
            userUV.isHidden = false
        default:
            break
        }
    }
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if(tableView.tag == 101){
            return favproductid.count
        }
        else{
            return favuserid.count
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if(tableView.tag == 101){
            let cell = self.favproductTB.dequeueReusableCell(withIdentifier: "cell") as! buyershopproductCell
            ...
            cell.productValue.text = oneproduct.price + "$"
            ...
            let cell = self.favuserTB.dequeueReusableCell(withIdentifier: "cell") as! buyerfavshopCell
            let oneshop : Shop
            ...
            cell.shopemail.text = oneshop.email
            cell.shopmobile.text = oneshop.mobile
            cell.favUV.layer.borderWidth = 1
            cell.favUV.layer.masksToBounds = false
            cell.favUV.layer.borderColor = UIColor.lightGray.cgColor
            cell.favUV.layer.cornerRadius = 10
            cell.favUV.clipsToBounds = true
            return cell
        }
